...
export const hooks = {
  ...
  beforeScenario: async function (uri, feature, scenario, sourceLocation) {
    // Clear all books before each scenario to ensure clean state
    // This prevents test interference from previous scenarios
    try {
      ...
      try {
        ...
        // Auth failed, skip clearing
        return;
      }
      
      // Clear books if we have a token
      if (token) {
        try {
          await axios.post(`${baseUrl}/api/books/reset`, {}, {
            headers: {
              Cookie: `token=${token}`,
